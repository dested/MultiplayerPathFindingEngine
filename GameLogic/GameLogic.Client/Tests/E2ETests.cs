...
namespace GameLogic.Client.Tests
{
    [TestClass()]
        ...
        public void Connect4(Deferred deferred)
        {
            ...
            for (int i = 0; i < 4; i++)
            {
                ...
                gameClient.ClientGameManager.OnReady += () =>
                {
                    Global.SetTimeout(() =>
                    {
                        ((LogicClientGameManager)gameClient.ClientGameManager).ClickLocation(point.X, point.Y);
                    }, 1000);
                    Global.SetInterval(() =>
                    {
                        ((LogicClientGameManager)gameClient.ClientGameManager).ClickLocation(point.X, point.Y);
                    ...
                    cl = Global.SetInterval(() =>
                    {
                        if (++receivedCount < 200)
                        {
                            while (true)
                            {
                                ...
                                var logicClientGameManager = ((LogicClientGameManager) gameClient.ClientGameManager);
                                var x = logicClientGameManager.clientGame.MyUser.X + ((Math.Random()*20) - 10)*Constants.SquareSize;
                                var y = logicClientGameManager.clientGame.MyUser.Y + ((Math.Random() * 20) - 10) * Constants.SquareSize;
