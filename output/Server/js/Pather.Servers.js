...
var $Pather_Servers_Common_BackEndTickManager = function(serverLogger) {
	...
	Pather.Common.Utils.TickManager.call(this, serverLogger);
};
...
var $Pather_Servers_GameSegmentServer_GameSegmentServer = function(pubsub, pushPop, gameSegmentId, instantiateLogic) {
	this.$pubsub = null;
	this.$pushPop = null;
	this.$gameSegmentId = null;
...
var $Pather_Servers_GameWorldServer_GameWorld = function(gameWorldPubSub, backEndTickManager, instantiateLogic, serverLogger) {
	...
	this.board = instantiateLogic.createGameBoard();
};
...
var $Pather_Servers_GatewayServer_GatewayServer = function(pubsub, pushPop, socketManager, gatewayId, port) {
	...
	Pather.Common.Utils.Promises.Q.all([pubsub.init(this.serverLogger, 6379), pushPop.init(this.serverLogger)]).then(ss.mkdel(this, function() {
		...
		this.backEndTickManager.init$1(ss.mkdel(this, this.$sendPing), ss.mkdel(this, function() {
			this.serverLogger.logDebug('Connected To Tick Server', []);
...
ss.initClass($Pather_Servers_ServerStarter, $asm, {
	...
	$ready: function(server, arguments1) {
		switch (server) {
			...
			case 'gt':
			case 'gateway': {
				this.$createGatewayServer(arguments1[3], parseInt(arguments1[4]));
				break;
			}
			case 'au':
			case 'auth': {
			...
			case 'cm':
			case 'clustermanager': {
				this.$createClusterManagerServer(arguments1[3]);
				break;
			}
			case 'gs':
			case 'gamesegment': {
				this.$createGameSegmentServer(arguments1[3]);
				break;
			}
			case 'sm':
			case 'servermanager': {
			...
			default: {
				this.$serverLogger.logError$1('Failed to load: ', [arguments1[2]]);
				break;
			}
		}
	},
	...
	$createGameWorldServer: function() {
		new $Pather_Servers_GameWorldServer_GameWorldServer(new $Pather_Servers_Common_PubSub_PubSub(), new $Pather_Servers_Database_DatabaseQueries(), $Pather_Servers_ServerStarter.instantiateLogic);
	},
	$createGameSegmentServer: function(gameSegmentId) {
		new $Pather_Servers_GameSegmentServer_GameSegmentServer(new $Pather_Servers_Common_PubSub_PubSub(), new $Pather_Servers_Common_PushPop_PushPop(), gameSegmentId, $Pather_Servers_ServerStarter.instantiateLogic);
	},
...
ss.initClass($Pather_Servers_ClusterManager_ClusterManager, $asm, {
	...
	$createGateway: function(createGatewayMessage) {
		...
		this.pushPop.blockingPop(createGatewayMessage.gatewayId, Pather.Common.Constants.gatewayCreationWait).then(ss.mkdel(this, function(content) {
			...
			this.serverLogger.logError$1('Gateway Server Creation Failed!', []);
		}));
	...
	$createGameSegment: function(createGameSegmentMessage) {
		...
		this.pushPop.blockingPop(createGameSegmentMessage.gameSegmentId, Pather.Common.Constants.gameSegmentCreationWait).then(ss.mkdel(this, function(content) {
			...
			this.serverLogger.logError$1('Game Segment Server Creation Failed!', []);
		}));
		this.$count++;
	...
	$startApp: function(arguments1, logFile) {
		...
		if (Pather.Common.Constants.dontSpawnNewApp) {
			...
			var child = spawn(str, arguments1, { stdio: [m, out, err] });
			//            child.Unref();
		}
	}
});
...
ss.initClass($Pather_Servers_Common_BackEndTickManager, $asm, {
	...
	onPongReceived: function() {
		if (ss.isNullOrUndefined(this.$pingSent)) {
			this.serverLogger.logError$1('Tried to send pong, but already completed ping cycle', []);
			return;
		}
...
ss.initClass($Pather_Servers_Common_PubSub_PubSub, $asm, {
	init: function(serverLogger, port) {
		...
		this.$subClient.on('ready', ss.mkdel(this, function() {
			...
			if (deferred.promise.isRejected || deferred.promise.isResolved) {
				if (ss.isValue(this.serverLogger)) {
					this.serverLogger.logError$1('Forced reconnect of redis', []);
				}
				return;
			}
		...
		this.$pubClient.on('error', ss.mkdel(this, function(err) {
			if (ss.isValue(this.serverLogger)) {
				this.serverLogger.logError$1('Error ' + err, []);
			}
		}));
		this.$subClient.on('error', ss.mkdel(this, function(err1) {
			if (ss.isValue(this.serverLogger)) {
				this.serverLogger.logError$1('Error ' + err1, []);
			}
		}));
	...
	receivedMessage: function(channel, message) {
		try {
			...
			if (ss.isValue(this.serverLogger)) {
				this.serverLogger.logError$1('Exception', [e, e.get_stack(), channel, message]);
			}
		}
	},
	dontLog: function() {
...
ss.initClass($Pather_Servers_Common_ServerLogging_ServerLogger, $asm, {
	...
	logError$1: function(item, jsonContent) {
	...
	logError: function(item, ex) {
		var m = [ex.get_message(), ex.get_stack(), ex.get_innerException()];
		Pather.Common.Utils.Logger.log(this.$serverType + this.$serverName, item, m, 'error');
		this.$pubsub.publish($Pather_Servers_Common_PubSub_PubSubChannels.serverLogger(this.$serverType), { serverType: this.$serverType, serverName: this.$serverName, message: item, content: m, logLevel: 'error', time: new Date() });
	},
...
}, null, [Pather.Common.Utils.IServerLogger]);
		...
		this.pubSub.subscribe($Pather_Servers_Common_PubSub_PubSubChannels.gameSegment$1(this.gameSegmentId), ss.mkdel(this, function(message1) {
			...
			if (Pather.Common.Utils.Utilities.hasField(Pather.Common.Models.GameSegment.Base.GameSegment_PubSub_ReqRes_Message).call(null, gameSegmentPubSubMessage1, function(m) {
				...
				if (!this.deferredMessages.containsKey(possibleMessageReqRes.messageId)) {
					this.$serverLogger.logError$1('Received message that I didnt ask for.', [message1]);
					return;
				}
...
ss.initClass($Pather_Servers_GameSegmentServer_ServerGame, $asm, {
	...
	serverProcessGameSegmentAction: function(user, action) {
		switch (action.gameSegmentActionType) {
			...
			case 'logicAction': {
				...
				this.processLogicAction(user, logicAction);
				break;
			}
			default: {
				throw new ss.ArgumentOutOfRangeException();
	...
	serverProcessTellGameSegmentAction: function(action) {
		switch (action.tellGameSegmentActionType) {
			...
			case 'logicAction': {
				var logicAction = action;
				this.processTellLogicAction(logicAction);
				break;
			}
			default: {
				throw new ss.ArgumentOutOfRangeException();
	...
	serverProcessNeighborGameSegmentAction: function(action) {
		switch (action.neighborGameSegmentActionType) {
			...
			case 'logicAction': {
				var logicAction = action;
				this.processNeighborLogicAction(logicAction);
				break;
			}
			default: {
				throw new ss.ArgumentOutOfRangeException();
	...
	userJoin: function(userJoinGameUser) {
		var serverGameUser = ss.cast(this.createGameUser(userJoinGameUser.userId), $Pather_Servers_GameSegmentServer_ServerGameUser);
		serverGameUser.gameSegment = this.gameManager.allGameSegments.get_item(this.gameManager.gameSegmentId);
	...
	tellUserJoin: function(message) {
		var serverGameUser = ss.cast(this.createGameUser(message.userId), $Pather_Servers_GameSegmentServer_ServerGameUser);
		serverGameUser.gameSegment = this.gameManager.allGameSegments.get_item(message.gameSegmentId);
	...
	diffNeighbors: function() {
		for (var $t1 = 0; $t1 < this.gameManager.myGameSegment.users.list.length; $t1++) {
			...
			if (added.length > 0 || removed.length > 0) {
				this.serverLogger.logDebug('Neighbors! ', [Pather.Common.Utils.EnumerableExtensions.select(added, function(a) {
					return a.entityId;
				}), Pather.Common.Utils.EnumerableExtensions.select(removed, function(a1) {
					return a1.entityId;
				})]);
				...
				$t8.removed = Pather.Common.Utils.EnumerableExtensions.select(removed, function(a2) {
					return a2.entityId;
				});
				$t8.entityId = serverGameUser.entityId;
				$t8.added = Pather.Common.Utils.EnumerableExtensions.select(added, ss.mkdel({ lockstepTickToRun: lockstepTickToRun, $this: this }, function(a3) {
					var inProgressActions = Pather.Common.Utils.EnumerableExtensions.where$1(a3.inProgressActions, ss.mkdel({ lockstepTickToRun: this.lockstepTickToRun }, function(action) {
						return action.endingLockStepTicking > this.lockstepTickToRun.$;
					}));
					this.$this.serverLogger.logDebug('In progress actions: ', [inProgressActions, a3.entityId]);
					var point;
					this.$this.serverLogger.logDebug('xy ', [a3.x, a3.y]);
					this.$this.serverLogger.logDebug('count ', [inProgressActions.length]);
					point = a3.getPositionAtLockstep(this.lockstepTickToRun.$);
					this.$this.serverLogger.logDebug('xy ', [point.x, point.y]);
					var $t9 = Pather.Common.Models.Common.UpdatedNeighbor.$ctor();
					$t9.userId = a3.entityId;
...
ss.initClass($Pather_Servers_GameSegmentServer_ServerGameManager, $asm, {
	...
	$onMessageTransferGameUser: function(message) {
		this.serverLogger.logInformation('Transfered users', [message.userId]);
	...
	$onMessageGameSegmentAction: function(message) {
		if (!this.myGameSegment.users.contains$1(message.userId)) {
			this.serverLogger.logError$1('This aint my user! ' + message.userId, []);
			return;
		}
...
ss.initClass($Pather_Servers_GameSegmentServer_ServerGameUser, $asm, {
	...
	lockstepTick: function(lockstepTickNumber) {
		...
		if (ss.keyExists(this.futureActions, lockstepTickNumber)) {
			...
			for (var $t1 = 0; $t1 < actions.length; $t1++) {
				var clientAction = actions[$t1];
				this.processAction(clientAction);
			}
			delete this.futureActions[lockstepTickNumber];
		}
	...
	addAction: function(action, lockstepTick) {
		if (!ss.keyExists(this.futureActions, lockstepTick)) {
			this.futureActions[lockstepTick] = [];
		}
		this.futureActions[lockstepTick].push(action);
	},
...
ss.initClass($Pather_Servers_GameWorldServer_DefaultInstanitateLogic, $asm, {
	...
	createGameBoard: function() {
		return new Pather.Common.GameFramework.GameBoard();
	}
...
ss.initClass($Pather_Servers_GameWorldServer_GameWorld, $asm, {
	...
	userLeft: function(dbUser) {
		...
		if (ss.isNullOrUndefined(gwUser)) {
			this.serverLogger.logError$1('IDK WHO THIS USER IS', [dbUser.token, dbUser.userId]);
			deferred.reject();
			...
			Pather.Common.Utils.Promises.Q.all$2(promises).then(ss.mkdel(this, function() {
				...
				this.serverLogger.logInformation('User left', [gwUser.userId]);
				deferred.resolve();
			}));
		}
		return deferred.promise;
	...
	gameWorldAction: function(gameWorldActionGameSegment) {
		switch (gameWorldActionGameSegment.action.gameWorldActionType) {
			case 'moveEntity': {
				...
				this.serverLogger.logData('Move entity:', [moveEntity.lockstepMovePoints]);
				var user = this.users.get_item(moveEntity.entityId);
				user.setLockstepMovePoints(moveEntity.lockstepMovePoints);
				break;
			}
			case 'logicAction': {
				var logicAction = gameWorldActionGameSegment.action;
	...
	init: function() {
		this.pubSub.subscribe($Pather_Servers_Common_PubSub_PubSubChannels.gameWorld(), ss.mkdel(this, function(message) {
			...
			if (Pather.Common.Utils.Utilities.hasField(Pather.Common.Models.Common.IPubSub_ReqRes_Message).call(null, gameWorldPubSubMessage, function(m) {
				...
				if (!this.$deferredMessages.containsKey(possibleMessageReqRes.messageId)) {
					this.serverLogger.logError$1('Received message that I didnt ask for.', [message]);
		...
		try {
			while ($t1.moveNext()) {
				...
				gameSegment.$.addUsersToSegment(items.$).then(ss.mkdel({ items: items, gameSegment: gameSegment, $this: this }, function(users) {
					if (users.length === 0) {
						this.$this.serverLogger.logError$1('No users to resolve adding to segment', [this.items.$]);
						return;
					}
		...
		switch (message.type) {
			case 'userJoined': {
				this.$userJoined(message).then(ss.mkdel(this, function(gwUser) {
					...
					$t1.gameSegmentId = gwUser.gameSegment.gameSegmentId;
					$t1.userId = gwUser.userId;
					$t1.grid = this.gameWorld.board.grid;
			...
			case 'initializeGameSegment': {
				...
				$t6.grid = this.gameWorld.board.grid;
...
ss.initClass($Pather_Servers_GameWorldServer_Models_GameWorldUser, $asm, {
	...
	lockstepTick: function(lockstepTickNumber) {
		if (ss.keyExists(this.lockstepMovePoints, lockstepTickNumber)) {
			...
			this.y = point.y;
			delete this.lockstepMovePoints[lockstepTickNumber];
			this.$gameWorld.serverLogger.logDebug(this.userId, [this.x, this.y, ss.getKeyCount(this.lockstepMovePoints), lockstepTickNumber]);
		}
	},
...
ss.initClass($Pather_Servers_GatewayServer_GatewayServer, $asm, {
	...
	$processLockStep: function(lockstepTickNumber) {
		if (ss.keyExists(this.$reorgUserAtLockstep, lockstepTickNumber)) {
			...
			this.serverLogger.logDebug('Reorg!', [reorgsThisTick]);
			for (var $t1 = 0; $t1 < reorgsThisTick.length; $t1++) {
				...
				if (ss.isNullOrUndefined(gatewayUser)) {
					this.serverLogger.logError$1('Tried to reorganize user who already left', [reorganizeUserMessage.userId]);
					continue;
				}
				this.serverLogger.logDebug('Old GS:', [gatewayUser.gameSegmentId, 'New GS:', reorganizeUserMessage.newGameSegmentId]);
				gatewayUser.gameSegmentId = reorganizeUserMessage.newGameSegmentId;
				gatewayUser.betweenReorgs = false;
				this.serverLogger.logDebug('Queued Messages:', [gatewayUser.queuedMessagesBetweenReorg.length]);
	...
	$onMessage: function(message) {
		...
		switch (message.type) {
			case 'userJoined': {
				...
				if (ss.isNullOrUndefined(gatewayUser)) {
					this.serverLogger.logError$1('User succsfully joined, but doesnt exist anymore', [userJoinedMessage.userId, userJoinedMessage.gameSegmentId]);
					var $t2 = this.gatewayPubSub;
			...
			case 'reorganizeUser': {
				...
				this.serverLogger.logInformation('Trying to reorg', [reorgUserMessage.userId, reorgUserMessage.newGameSegmentId, reorgUserMessage.switchAtLockstepNumber]);
	...
	$processClientAction: function(clientActionMessage) {
		...
		for (var $t1 = 0; $t1 < clientActionMessage.users.length; $t1++) {
			...
			if (ss.isNullOrUndefined(gatewayUser)) {
				...
				this.serverLogger.logError$1('User does not exist yet, storing actions', [userToSendTo]);
		...
		switch (message.userGatewayMessageType) {
			...
			case 'gameSegmentAction': {
				...
				if (user.betweenReorgs) {
					this.serverLogger.logInformation('Adding message to reorg queue:', [user.userId]);
	...
	$shouldSpinUpNewGateway: function() {
		if (this.$isCurrentlySpawning === 0) {
			...
			if (this.$isCurrentlySpawning === 10) {
				this.$isCurrentlySpawning = 0;
				this.serverLogger.logError$1('Failed to create a new gateway.', []);
			}
		}
	},
	...
	$spawnNewServer: function() {
		...
		if (Pather.Common.ConnectionConstants.get_production()) {
			...
			this.pushPop.blockingPop(applicationId, Pather.Common.Constants.gameSegmentCreationWait).then(ss.mkdel(this, function(content) {
				...
				this.serverLogger.logError$1('Spawn Fail', []);
			}));
...
ss.initClass($Pather_Servers_Utils_Linode_LinodeBuilder, $asm, {
	...
	create: function(name, image, planId) {
		...
		$t7.thenPromise$1(Object).call($t7, ss.mkdel(this, function(res6) {
			...
			this.$serverLogger.logError$1('Linode', [err]);
			deferred.reject(err);
		}));
		return deferred.promise;
	},
...
(function() {
	$Pather_Servers_ServerStarter.instantiateLogic = null;
})();
(function() {
